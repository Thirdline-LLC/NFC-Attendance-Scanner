...
- **Desk:** subtitle is `name · typeLabel` for a root and `Parent › Child · typeLabel` for a nested body. No body control on the scanner.
  - *Amended by [Design 09](09-multi-period-classes-and-range-export.md) §3 (slice 4):* the scanner has one body control, a **sibling-only period switcher**. It is shown only when the active body has a parent with at least one other non-archived child, and it lists only those siblings: never the parent, other roots, cousins, deeper descendants or archived bodies. It is disabled while any tap is pending, and it switches through the scan queue, so the no-flip-mid-queue rule above still holds. It shows no roster, history or figures, and each switch logs `body-switch` with body ids and names only. An optional per-device **Require PIN to switch periods** setting asks for the teacher PIN first. The picker also marks a collapsed group that holds the active body ("Current: Period 3").
- **Dashboard:** toggle “This body” / “This body + descendants.” The roll-up states the email-then-card rule. Unknown cards say they are summed per body. The active-body card uses generic copy. *Design 09 §2 (slice 3):* with the subtree toggle on a body that has children, a per-child breakdown table follows the roll-up. *Design 09 §5 (slice 6):* the **subtree export half of 08c** is pulled forward: the Export dialog's **This body + all periods** scope writes the body plus every descendant (any depth, archived included and marked) with a Period (Body Path) column and a Summary by period sheet; class totals use the email-then-card roll-up rule. The 08c import half (multi-body `.nfc-pack`, Body Path / Parent Body import columns) stays deferred.
