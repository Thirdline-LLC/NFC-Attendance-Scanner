...
**Status:** Spec 2026-09-24 · Build-order step 3 (§1 setup flow, §2 tree + per-period breakdown) implemented on `feat/d09-class-with-periods` · Step 4 (§3 scanner period switcher, switch-PIN setting) implemented on `feat/d09-period-switcher`  
...
4. **Scanner period switcher** + Design 08 amendment. **Implemented** (`feat/d09-period-switcher`). Choices made there:
   ...
   - **Session join:** `findTodaysSessionForBody` looks at the new body's own taps; if it has one on today's meeting date (the local date `formatSessionDate` files sessions under), the desk joins the session of its latest such tap (`adoptSessionId`). Otherwise it calls `createNewSessionId`, the same call Start New Session makes, so the new body never shares a session id with the one just left. The old body's session is untouched; switching back the same day rejoins it. The lookup runs before `setActiveBody`, and the session id is only changed after the switch is accepted, so a refused switch changes nothing. Switching from the dashboard picker is unchanged (it does not rotate or join sessions).
