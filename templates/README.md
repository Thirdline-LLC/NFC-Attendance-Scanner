...
As a safety net, the example rows' graduation years are written as `2028 (example)` / `2027 (example)`. The importer only accepts a plain four-digit year, so it refuses a leftover example row, even with its email cleared. The rest of the file still imports. Your own rows need a plain year such as `2028`.

- **Required columns:** `first_name`, `last_name`, `grad_year`.
- **Optional columns:**
  - `email`: used to match a student on re-import.
  - `body_name` / `body_type`: must match the active body, or the whole file is refused. Leave both blank to skip that check.
